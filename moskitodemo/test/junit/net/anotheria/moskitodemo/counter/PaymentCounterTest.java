...
public class PaymentCounterTest {
	@Test public void testDifferentPaymentMethods() throws OnDemandStatsProducerException{
		...
		IStatsProducer<CounterStats> producer = registry.getProducer("PaymentCounter");
		...
		assertEquals("electroniccash", producer.getStats().get(1).getName());

		assertEquals(1, producer.getStats().get(2).get());
		assertEquals("creditcard", producer.getStats().get(2).getName());
		...
		assertEquals(2, onDemandProducer.getStats("electroniccash").get());
		assertEquals(1, onDemandProducer.getStats("creditcard").get());
